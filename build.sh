...
Export OpenC906 smart_run runner artifacts (Verilator backend).
...
if [[ "${CORES}" != "1" ]]; then
  echo "ERROR: OpenC906 supports --cores 1 only (got: ${CORES})" >&2
  exit 2
fi
if [[ "${COVERAGE_MODE}" != "none" ]]; then
  echo "ERROR: OpenC906 coverage builds are not supported yet" >&2
  exit 2
fi
...
build_verilator() {
  ...
  echo "[openc906] verilating with CX_TRACE..."
  make -C "${SMART_DIR}" compile \
    SIM=verilator \
    THREADS="${CX_VERILATOR_THREADS:-4}" \
    CODE_BASE_PATH="${ROOT_DIR}/C906_RTL_FACTORY" \
    SIMULATOR_DEF="-cc --exe --top-module top +define+CX_TRACE"
  echo "[openc906] compiling Vtop..."
