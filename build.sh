...
if [[ "${CORES}" != "1" ]]; then
  echo "ERROR: OpenC906 supports --cores 1 only (got: ${CORES}); RTL has no second core" >&2
  exit 2
fi
...
build_verilator() {
  ...
  echo "[openc906] verilating with CX_TRACE (mode=${mode})..."
  ...
  echo "[openc906] compiling Vtop (mode=${mode})..."
...
emit_runner() {
  ...
  local artifact_name="openc906_${isa}_${CORES}c${suffix}"
