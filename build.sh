...
Export OpenC906 smart_run runner artifacts.
...
if [[ "${CORES}" != "1" ]]; then
  echo "ERROR: OpenC906 integration currently supports --cores 1 only (got: ${CORES})" >&2
  exit 2
fi
if [[ "${COVERAGE_MODE}" != "none" ]]; then
  echo "ERROR: OpenC906 coverage builds are not supported by the current smart_run backend" >&2
  exit 2
fi
...
validate_isa() {
  case "$1" in
    rv64|rv64f|rv64fd) ;;
    *) echo "ERROR: OpenC906 integration supports --isa rv64, rv64f, rv64fd only (got: $1)" >&2; exit 2 ;;
  esac
}

emit_runner() {
  local isa="$1"
  local artifact_name="openc906_${isa}_${CORES}c"
...
SELF="$(readlink -f "$0")"
CORE_ROOT="${CX_OPENC906_ROOT:-$(cd "$(dirname "${SELF}")/../cores/openc906" 2>/dev/null && pwd || true)}"
...
TRACE_DIR=""
RUN_DIR=""
TIMEOUT_SEC="${CX_OPENCORE_TIMEOUT_SEC:-120}"
KEEP=0
...
[[ -d "${CORE_ROOT}/smart_run" ]] || { echo "ERROR: OpenC906 smart_run not found under ${CORE_ROOT}" >&2; exit 2; }
...
RUN_DIR="${RUN_DIR:-$(mktemp -d /tmp/openc906-run.XXXXXX)}"
...
TRACE_FILE="${TRACE_DIR}/openc906_trace_hart_00000000.log"
...
if [[ ! -f "${SMART}/work/xuantie_core.vvp" || "${TB_FILE}" -nt "${SMART}/work/xuantie_core.vvp" ]]; then
  mkdir -p "${SMART}/work"
  make -C "${SMART}" compile SIM="${SIM}" CODE_BASE_PATH="${CORE_ROOT}/C906_RTL_FACTORY" SIMULATOR_DEF="-DIVERILOG_SIM -DCX_TRACE"
fi
