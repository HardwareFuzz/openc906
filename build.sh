...
usage() {
  cat <<'EOF'
Usage: ./build.sh [--isa rv64f|rv64fd]... [--cores 1]
                  [--coverage|--coverage-light|--no-coverage]
                  [--out-dir DIR] [--clean]

Export OpenC906 smart_run Verilator binaries.
...
validate_isa() {
  case "$1" in
    rv64f|rv64fd) ;;
    *) echo "ERROR: OpenC906 integration supports --isa rv64f, rv64fd only (got: $1)" >&2; exit 2 ;;
  esac
}
